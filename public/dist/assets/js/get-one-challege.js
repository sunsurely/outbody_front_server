...
async function getChallengeDetail() {
  axios
    .get(`http://3.34.131.11:3000/challenge/${challengeId}`, {
      headers: {
...
async function getChallengers() {
  axios
    .get(`http://3.34.131.11:3000/challenge/${challengeId}/challengers`, {
      headers: {
        Authorization: accessToken,
      },
    })
...
document.addEventListener('click', async (event) => {
  ...
  if (target.matches('#enter-challenge')) {
    await axios
      .post(`http://3.34.131.11:3000/challenge/${challengeId}/enter`, null, {
        headers: {
          Authorization: accessToken,
        },
      })
...
document.addEventListener('click', async (event) => {
  ...
  if (target.matches('#leave-challenge')) {
    ...
    if (leaveConfirm) {
      await axios
        .delete(`http://3.34.131.11:3000/challenge/${challengeId}/leave`, {
          headers: {
            Authorization: accessToken,
          },
        })
...
document.addEventListener('click', async (event) => {
  ...
  if (target.matches('#delete-challenge')) {
    ...
    if (deleteConfirm) {
      await axios
        .delete(`http://3.34.131.11:3000/challenge/${challengeId}`, {
          headers: {
            Authorization: accessToken,
          },
        })
...
$('#send-invitation-button').on('click', async () => {
  ...
  try {
    const response = await axios.get(
      `http://3.34.131.11:3000/user/me/searchEmail/?email=${emailInput}`,
      {
        headers: {
          Authorization: accessToken,
        },
    ...
    $('#send-invitation').on('click', async () => {
      ...
      await axios
        .post(`http://3.34.131.11:3000/challenge/${challengeId}/invite`, data, {
          headers: {
            Authorization: accessToken,
          },
        })
